...
function buildBreadcrumb(id, status, templates) {
  ...
  return `<workflow-state>\n${header}\n${body}\n<Action-Now>\n1. Follow workflow within <skill></skill>.\n2. Run or document, or run and document.\n</Action-Now>\n</workflow-state>`
}
      ...
      "chat.message": async (input, output) => {
        try {
          ...
          if (SUBAGENT_TYPES.includes(agent)) {
            debugLog("workflow-state", "Skipping - subagent session:", agent)
            return
          }
          ...
          if (textPartIndex !== -1) {
            ...
            parts.push({ type: "text", text: breadcrumb })
          }
          debugLog(
            "workflow-state",
            "Injected breadcrumb",
            task ? `task=${task.id} status=${task.status}` : "no_task",
          )
        } catch (error) {
          debugLog(
            "workflow-state",
